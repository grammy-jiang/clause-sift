...
| `requirement`, `clause`, `subclause`, `paragraph`, or `table_row` | `depends_on` forward | required | Include every uniquely resolved definition, requirement, clause, table, or document dependency. Apply required rules recursively. A definition required by actual term use must be compiled as this edge; runtime reverse-scanning of all `defines` scopes is forbidden. |
...
| `exception` | `exception_to` forward | required | Include the exact affected source-bearing node and stop this relation after that target; the exception's other required rules, including forward `applies_subject_to`, still execute. Cycles are release-invalid. |
...
Both source-node fields, when present, belong to `source_document_id` through separate composite foreign keys. For every source-text relation except `supersedes` and `amends`, `source_node_id` must equal `occurrence_node_id` and that node must satisfy the relation's source category. A source-text `supersedes` proposal deterministically uses the newer document's sole root as `source_node_id` regardless of which source-bearing node contains the phrase. For source-text `amends`, the logical source is the occurrence node when it already satisfies the endpoint category; otherwise it is the nearest addressable clause ancestor, or the sole document root when there is none. Release validation recomputes these projections from the immutable tree and rejects a missing anchor, cross-document anchor, non-source-bearing anchor, wrong root/ancestor, or occurrence slice mismatch. A resolved reference stores only the release-scoped `target_document_id` and `target_node_id`; document code, edition, and clause projections alone are not stable join keys. Document resolution is deterministic:
...
- every Section 20 relationship direction, exact Section 12.2 endpoint category, origin, cross-document rule, semantic identity, source-endpoint derivation, and cycle policy; byte-exact v1 edge-ID derivation for `contains`, `precedes`, and every semantic relation; stability across identical rebuilds and repeated semantic occurrences; inequality after a changed relation type, endpoint, or identity-schema version; and rejection of a malformed prefix, wrong digest, collision, endpoint mismatch, or unresolved occurrence carrying an edge ID. Origin fixtures require the exact empty structural group, one-source-origin semantic groups, and a coalesced `source_text` plus `manifest` edge with both non-empty occurrence groups and no lost provenance. Other fixtures cover paragraph sources for `depends_on` and `applies_subject_to`; exception-source applicability reached after reverse `exception_to`; paragraph and subclause applicability targets; structural/document/appendix applicability scopes; a paragraph-anchored supersession phrase normalized to the newer root and coalesced only with the matching manifest-authorized root-to-root edge; rejection or non-navigation for a missing manifest edge, wrong occurrence anchor, mismatched supersession target, or incorrect amendment ancestor projection; paragraph targets for reverse `exception_to`; rejection of `condition` as a non-canonical endpoint token; duplicate occurrences preserved behind one runtime edge; explicit multi-target citations split into individual edges; ambiguous candidates producing no edge; allowed bounded reference/dependency cycles; forbidden structural/governing cycles; `unclassified` exclusion from typed endpoints; and unknown relation types failing release validation;
- every Section 20.3 candidate/state/dimension/explanation/precedence rule using exact unit conversions and modality-set intersections; fixtures cover compatible stricter minima/maxima, empty intersections, unit equivalence, exception, amendment, supersession, disjoint and missing jurisdiction/effective interval/equipment class, `required` versus `recommended` provisions across code/standard/manufacturer document types, two- and three-position records, and natural-language candidates that remain unresolved without immutable review;
