...
The initial `resolution_status` enum is `resolved`, `unresolved_document`, `ambiguous_edition`, `unresolved_clause`, `unresolved_node`, and `ambiguous_node`. Same-document references inherit the source `document_id`. `resolved` requires both target IDs. A missing addressable clause produces `unresolved_clause`; a canonical-node rule yielding no valid endpoint produces `unresolved_node`; and one yielding several candidates produces `ambiguous_node`. A resolved canonical-node target must exist in the selected document, satisfy any clause scope and the relation's endpoint category, and equal the independently recomputed proposed ID. Release validation reruns both the deterministic four-step document resolver and the tagged node selector from immutable parsed fields, approved edition overrides, source occurrence, selector provenance, and candidate release catalog. It requires the stored `resolution_status`, `target_document_id`, and `target_node_id` to equal the result exactly. For a `resolved` result only, every non-null parsed document code and edition must equal the selected joined document values. Every non-resolved result instead requires both target IDs and all joined target projections to be null; its non-null parsed fields and valid selector evidence remain non-navigable extraction evidence. A proposed node from another document, a node outside its parsed clause scope, a disallowed endpoint type, a writable code/edition projection, joined target data on an unresolved result, or target IDs attached to an unresolved result blocks index assembly and activation.
...
| Evidence lineage | Approved manifest-content and exact source hashes/sizes; exact evidence-vocabulary artifact hash; ordered selected parser-route provenance-envelope hashes; passing parser-validation-report hash; canonical-model/classification-provenance, page-provenance, chunk/source, catalog, cross-reference, conflict-analysis, embedding, lexical-index, and vector-index artifact hashes; lineage and edge-identity schema versions; context/conflict rule-set, review, ordering and configuration hashes. The output excludes its own hash, `build_content_id`, `release_id`, operational IDs, and timestamps. |
...
- edge-identity schema version plus context rule-set, ordering, limit values, and canonical configuration hash;
...
- cross-reference resolution for same-document, exact-edition, manifest-override, unqualified-unique, and two-edition-ambiguous cases; all three tagged selectors; same- and cross-document canonical-node selectors for paragraph, requirement, exception, and table-row targets; optional addressable-clause scoping; and missing or ambiguous node candidates. Fixtures independently recompute selectors and reject an existing but semantically wrong target node, wrong document ownership, out-of-clause target, wrong endpoint category, missing or cross-document occurrence anchor, altered occurrence span, wrong logical source-node projection, stale rule/configuration/provenance, manifest-origin non-addressable selector, wrong code or edition, and wrong document-root target. Resolved rows compare parsed code and edition with joined targets, while unresolved rows retain non-navigable parsed/selector evidence with null target IDs/projections and perform no joined-field equality check;
- every Section 20 relationship direction, exact Section 12.2 endpoint category, origin, cross-document rule, semantic identity, source-endpoint derivation, and cycle policy; byte-exact v1 edge-ID derivation for `contains`, `precedes`, and every semantic relation; stability across identical rebuilds and repeated semantic occurrences; inequality after a changed relation type, endpoint, or identity-schema version; and rejection of a malformed prefix, wrong digest, collision, endpoint mismatch, or unresolved occurrence carrying an edge ID. Origin fixtures require the exact empty structural group, one-source-origin semantic groups, and a coalesced `source_text` plus `manifest` edge with both non-empty occurrence groups and no lost provenance. Other fixtures cover paragraph sources for `depends_on` and `applies_subject_to`; exception-source applicability and dependency traversal reached after reverse `exception_to`, including a `subclause` dependency target; paragraph and subclause applicability targets; structural/document/appendix applicability scopes; paragraph-anchored supersession and amendment phrases normalized to their source document roots, with supersession coalesced only with the matching manifest-authorized root-to-root edge; rejection or non-navigation for a missing manifest edge, wrong occurrence anchor, mismatched supersession target, or non-root version-edge source projection; paragraph targets for reverse `exception_to`; rejection of `condition` as a non-canonical endpoint token; duplicate occurrences preserved behind one runtime edge; explicit multi-target citations split into individual edges; ambiguous candidates producing no edge; allowed bounded reference/dependency cycles; forbidden structural/governing cycles; `unclassified` exclusion from typed endpoints; and unknown relation types failing release validation;
