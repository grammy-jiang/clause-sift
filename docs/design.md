...
Human-reviewed `relations` entries are authoritative for `supersedes`; `superseded_by` is their validated inverse view. An extracted supersession statement is stored as a `source_text` occurrence whose evidence anchor and exact offsets may be in a paragraph while its logical source endpoint is the newer document root. Target resolution alone does not authorize a supersession edge: only an approved manifest occurrence admits the root-to-root edge. A uniquely resolved source-text occurrence that names that exact manifested pair is reconciled and coalesced only as additional provenance; without the matching manifest edge it remains non-navigable proposed metadata. An invalid logical source root fails release validation. A different target or edition never creates a competing source-text edge, emits `edition_conflict`, and blocks a `release_tier: critical` document until reviewed.
