...
| --- | --- | --- | --- |
| `requirement`, `clause`, `subclause`, `paragraph`, or `table_row` | `applies_subject_to` forward | required | Include every uniquely resolved applicability target whose node type is admitted by Section 20.1. Apply required rules recursively to the target. |
...
| `applies_subject_to` | A governed requirement, clause, subclause, paragraph, or table row → an explicit applicability target of canonical type `document`, `part`, `chapter`, `section`, `clause`, `subclause`, `paragraph`, `requirement`, `exception`, `table`, `table_row`, or `appendix`. | The target must contain or represent the source-grounded applicability qualifier. Cross-document only for an explicit, uniquely resolved target. `condition` is a semantic description, not a node-type token. | `source_text` or `manifest`; eligible for applicability context. Self-edges and same-relation cycles are invalid. |
...
- cross-reference resolution for same-document, exact-edition, manifest-override, unqualified-unique, and two-edition-ambiguous cases; all three tagged selectors; same- and cross-document canonical-node selectors for paragraph, requirement, exception, and table-row targets; optional addressable-clause scoping; and missing or ambiguous node candidates. Fixtures independently recompute selectors and reject an existing but semantically wrong target node, wrong document ownership, out-of-clause target, wrong endpoint category, altered occurrence span, stale rule/configuration/provenance, manifest-origin non-addressable selector, wrong code or edition, and wrong document-root target. Resolved rows compare parsed code and edition with joined targets, while unresolved rows retain non-navigable parsed/selector evidence with null target IDs/projections and perform no joined-field equality check;
- every Section 20 relationship direction, exact Section 12.2 endpoint category, origin, cross-document rule, semantic identity, and cycle policy; byte-exact v1 edge-ID derivation for `contains`, `precedes`, and every semantic relation; stability across identical rebuilds and repeated semantic occurrences; inequality after a changed relation type, endpoint, or identity-schema version; and rejection of a malformed prefix, wrong digest, collision, endpoint mismatch, or unresolved occurrence carrying an edge ID. Origin fixtures require the exact empty structural group, one-source-origin semantic groups, and a coalesced `source_text` plus `manifest` edge with both non-empty occurrence groups and no lost provenance. Other fixtures cover paragraph sources for `depends_on` and `applies_subject_to`, paragraph and subclause applicability targets, structural/document/appendix applicability scopes, paragraph targets for reverse `exception_to`, rejection of `condition` as a non-canonical endpoint token, duplicate occurrences preserved behind one runtime edge, explicit multi-target citations split into individual edges, ambiguous candidates producing no edge, allowed bounded reference/dependency cycles, forbidden structural/governing cycles, `unclassified` exclusion from typed endpoints, and unknown relation types failing release validation;
