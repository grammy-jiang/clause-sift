...
| Evidence lineage | Approved manifest-content and exact source hashes/sizes; exact evidence-vocabulary artifact hash; ordered selected parser-route provenance-envelope hashes; passing parser-validation-report hash; canonical-model/classification-provenance, page-provenance, chunk/source, catalog, cross-reference, conflict-analysis, embedding, lexical-index, and vector-index artifact hashes; lineage schema version; context/conflict rule-set, review, ordering and configuration hashes. The output excludes its own hash, `build_content_id`, `release_id`, operational IDs, and timestamps. |
...
- context rule-set, ordering, limit values, and canonical configuration hash;
...
- Evidence Lineage schema closure and canonical ordering: exact source/manifest/vocabulary hashes and sizes, every contributing node's three classification records and provenance hashes, ordered multi-node and multi-page byte spans, coordinate-status derivation, selected parser roles and transformation hashes, exact context and conflict rule-set version/configuration bindings, chunk/source ownership, retrieval-artifact references, direct/expanded/conflict selection roles, and unique independent context paths/reasons; require a critical/comparison-mode document to carry ordered distinct primary/comparator artifacts plus a `passed_*` status, allow `not_required` only for an approved standard-tier single route, retain otherwise identical edge sequences when their context classes differ, and reject a missing, extra, renamed, or release-mismatched rule-set field, any other missing or unknown field, wrong classification/value/origin/order, wrong ownership, a nonexistent artifact/edge/occurrence, unresolved-target traversal, an exact duplicate `(seed source, context class, edge-ID sequence)` path, a direct-only item with a path, or a context-only item without one;
