...
An accepted target is materialized as source-backed Evidence Package items rather than generated text. The versioned rule declares its evidence scope: semantic references to an addressable clause/subclause use the byte-complete retrievable subtree defined in Section 14.1; atomic requirements, definitions, exceptions, notes, footnotes, and rows use their exact node interval; table context uses its validated whole-table representation; structural ancestors use only their own non-empty source interval; and a document root used for version context contributes safe document metadata and a path but never expands the whole document. A source is eligible for that target only when every membership interval contributing to its chunk lies within the declared scope; its complete `original_text` is therefore reconstructed only from in-scope source spans and their declared separator. A source already selected for another seed or target satisfies the new target only when it passes this same containment test; a broader source remains independently valid but neither receives the narrower target's inclusion path nor suppresses its scope-contained cover. At the first uncovered scoped byte, the runtime considers memberships from eligible sources satisfying `node_text_start <= uncovered < node_text_end`, selects the one with greatest `node_text_end`, breaks ties by `(chunk-kind rank, chunk canonical_order, chunk_id)`, and repeats until the declared scope is covered. Consequently, a multi-node chunk containing an atomic target plus any neighboring requirement, exception, definition, note, footnote, or row cannot materialize that atomic target. A structural node with no source text contributes its typed path and metadata but no invented evidence item. Release validation executes this scope-contained algorithm for every node eligible under a required rule and blocks a release with an incomplete cover; table-kind ranking prefers the whole-table representation needed to preserve headers and units. A source eligible for several seeds or targets appears once and retains every unique lineage path; overlapping sources required by different scopes remain separate, and each uses the smallest applicable queue tuple as its output-order key. Direct seeds precede expanded items; expanded items then sort by that retained key and finally `source_id`.
