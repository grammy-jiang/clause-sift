...
For `contains` and `precedes`, the identity fields come directly from the release-validated node ownership/sequence relations. For a semantic relation, they are exactly the normalized logical edge tuple after unique resolution; occurrence IDs and their order are provenance and are deliberately excluded from logical edge identity. Repeated citations with that tuple normalize to one runtime edge whose ordered provenance list contains every occurrence, while an unresolved occurrence has no edge or `edge_id`. The relationship artifact exposes the derived IDs for semantic edges, and structural IDs remain deterministic derived views. The builder, release validator, startup validator, and central lineage serializer independently recompute every ID, reject a value with the wrong prefix/digest or an identity collision, and require every serialized path step to match the recomputed ID and endpoints. The edge-identity schema version is a cross-reference/context cache input and a release-manifest/build-identity field.
