...
- Evidence Lineage schema closure and canonical ordering: exact source/manifest/vocabulary hashes and sizes, every contributing node's three classification records and provenance hashes, ordered multi-node and multi-page byte spans, coordinate-status derivation, selected parser roles and transformation hashes, chunk/source ownership, retrieval-artifact references, direct/expanded/conflict selection roles, and unique independent context paths/reasons; require a critical/comparison-mode document to carry ordered distinct primary/comparator artifacts plus a `passed_*` status, allow `not_required` only for an approved standard-tier single route, and reject missing or unknown fields, wrong classification/value/origin/order, wrong ownership, a nonexistent artifact/edge/occurrence, unresolved-target traversal, duplicate paths, a direct-only item with a path, or a context-only item without one;
