...
An accepted source-bearing target is materialized as source-backed Evidence Package items rather than generated text. The versioned rule declares its evidence scope: semantic references to an addressable clause/subclause use the byte-complete retrievable subtree defined in Section 14.1; atomic requirements, definitions, exceptions, notes, footnotes, and rows use their exact node interval; table context uses its validated whole-table representation; and source-bearing structural ancestors use only their own non-empty source interval. A source is eligible for that target only when every membership interval contributing to its chunk lies within the declared scope; its complete `original_text` is therefore reconstructed only from in-scope source spans and their declared separator. A source already selected for another seed or target satisfies the new target only when it passes this same containment test; a broader source remains independently valid but neither receives the narrower target's inclusion path nor suppresses its scope-contained cover. At the first uncovered scoped byte, the runtime considers memberships from eligible sources satisfying `node_text_start <= uncovered < node_text_end`, selects the one with greatest `node_text_end`, breaks ties by `(chunk-kind rank, chunk canonical_order, chunk_id)`, and repeats until the declared scope is covered. Consequently, a multi-node chunk containing an atomic target plus any neighboring requirement, exception, definition, note, footnote, or row cannot materialize that atomic target.
...
| `get_clause` | Exact opaque `document_id` plus normalized exact `clause_number`; no fuzzy clause or edition substitution. Resolve the canonical clause node and select every distinct persisted chunk needed to cover its retrievable subtree, then run required graph-and-conflict closure from every direct covering source. | `{release, context_completeness, evidence, context_targets, conflicts, warnings}` with a non-empty, canonically ordered source-backed `evidence` array and a possibly empty metadata-target array. Exactly the covering chunks are `retrieval_seed`; later evidence items are distinguishable `expanded_context` or `conflict_context`. Every evidence item retains its own `source_id` and source span. | `identifier_invalid` for malformed input; `resource_not_found` when the document or clause is absent; `context_limit_exceeded` when complete required graph/conflict closure cannot fit a declared bound. |
